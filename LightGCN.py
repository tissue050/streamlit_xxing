...
# load edges between users and songs
def load_edge_excel(df,
                  src_index_col,
                  dst_index_col,
                  link_index_col,
                  rating_threshold=1):
    ...
    dst = [song_id for song_id in df['song_id']]
...
def convert_r_mat_edge_index_to_adj_mat_edge_index(input_edge_index):
    R = torch.zeros((num_users, num_songs))
    ...
    adj_mat = torch.zeros((num_users + num_songs , num_users + num_songs))
...
def convert_adj_mat_edge_index_to_r_mat_edge_index(input_edge_index):
    sparse_input_edge_index = SparseTensor(row=input_edge_index[0],
                                           ...
                                           sparse_sizes=((num_users + num_songs), num_users + num_songs))
...
def sample_mini_batch(batch_size, edge_index):
    ...
    edges = structured_negative_sampling(edge_index) # gives a tuple
...
def get_metrics(model,
    ...
    exclude_edge_indices = [convert_adj_mat_edge_index_to_r_mat_edge_index(exclude_edge_index)                                       for exclude_edge_index in input_exclude_edge_indices]
...
def get_embs_for_bpr(model, input_edge_index):
    ...
    #we can use r_mat_edge_index because embeddings are seperate, hence indexing starts from 0 for both users and items
    users_emb_final, users_emb_0 = users_emb_final[user_indices], users_emb_0[user_indices]
    pos_items_emb_final, pos_items_emb_0 = items_emb_final[pos_item_indices], items_emb_0[pos_item_indices]
    neg_items_emb_final, neg_items_emb_0 = items_emb_final[neg_item_indices], items_emb_0[neg_item_indices]
...
def make_predictions(user_id,model,rating_df):
    e_u = np.array(model.users_emb.weight.cpu().detach().numpy())[user_id]

    scores = np.dot(model.items_emb.weight.cpu().detach().numpy(), e_u)
    
    edge_index = load_edge_excel(
    rating_df,
    ...
    exclude_items = user_pos_items[user_id]
    scores[exclude_items] = -(1 << 10)
    
    indices = np.argsort(scores)[::-1]
    #sorted_items = [(i, scores[i]) for i in indices]
    sorted_items = [i for i in indices]
    return sorted_items
...
def calculate_rank(lst):
    ...
    for i, val in enumerate(lst):
        rank_dict[val].append(i + 1)  # Add 1 to convert from 0-based index to 1-based rank
    rank = {val: sum(rank_list) / len(rank_list) for val, rank_list in rank_dict.items()}
    return rank

def merge_and_sort(lst1, lst2, weight1=0.7, weight2=0.3):
    rank1 = calculate_rank(lst1)
    rank2 = calculate_rank(lst2)
    
    merged_rank = {}
    for val in set(rank1.keys()) | set(rank2.keys()):
        rank_val1 = rank1.get(val, 0)
        rank_val2 = rank2.get(val, 0)
        merged_rank[val] = (rank_val1 * weight1) + (rank_val2 * weight2)
    
    sorted_lst = sorted(merged_rank, key=lambda x: merged_rank[x])
    return sorted_lst
